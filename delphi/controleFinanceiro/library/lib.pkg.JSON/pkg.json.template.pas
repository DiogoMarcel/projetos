...
type
  TJsonPkgTemplate = class(TInterfacedObject, IPkgJsonTemplate)
  private
...
type
  TGenericListReflectAttribute = class(JsonReflectAttribute)
  public
    constructor Create;
  end;
...
constructor TJsonPkgTemplate.Create;
begin
  inherited;
  FOptions := [joIgnoreEmptyArrays, joDateIsUTC, joDateFormatISO8601];
end;

function TJsonPkgTemplate.GetAsJson: string;
begin
  Result := TJson.ObjectToJsonString(Self, FOptions);
end;

class procedure TJsonPkgTemplate.PrettyPrintJSON(aJSONValue: TJsonValue; aOutputStrings: TStrings; Indent: Integer);
var
  i: Integer;
  Ident: Integer;
begin
...
class procedure TJsonPkgTemplate.PrettyPrintArray(aJSONValue: TJSONArray; aOutputStrings: TStrings; Last: Boolean; Indent: Integer);
var
  i: Integer;
begin
...
class function TJsonPkgTemplate.PrettyPrintJSON(aJson: string): string;
var
...
class procedure TJsonPkgTemplate.PrettyPrintPair(aJSONValue: TJSONPair; aOutputStrings: TStrings; Last: Boolean; Indent: Integer);
const
  TEMPLATE = '%s:%s';
var
  Line: string;
...
procedure TJsonPkgTemplate.SetAsJson(aValue: string);
var
  JSONValue: TJsonValue;
...
{ TGenericListReflectAttribute }
constructor TGenericListReflectAttribute.Create;
begin
