...
type
  ...
  private
    ...
    function PegarSQLProjecao: string;
...
begin
  case FSQLEnum of
    ...
    sqlMenuProjecao       : Result := PegarSQLProjecao;
    else
...
function TSQLResumoMenu.SQL_SaldoBancario: string;
begin
...
function TSQLResumoMenu.SQL_SaldoMensal: string;
begin
...
function TSQLResumoMenu.SQL_Receita: string;
begin
...
function TSQLResumoMenu.SQL_Despesa: string;
begin
...
function TSQLResumoMenu.SQL_SaldoFGTS: string;
begin
...
function TSQLResumoMenu.SQL_ValorReservado: string;
begin
...
function TSQLResumoMenu.SQL_DivAtualAnual: string;
begin
...
function TSQLResumoMenu.SQL_DifReservado: string;
begin
...
function TSQLResumoMenu.SQL_SaldoTotal: string;
begin
...
function TSQLResumoMenu.SQL_SaldoGeralTot: string;
begin
...
begin
  Result := 'SELECT CAST(''Projeção do Saldo Mensal'' AS VARCHAR(40)) AS TITULO         '+
            '   ,COALESCE((                                                             '+
            '       SUM(CASE                                                            '+
            '           WHEN C.TIPOCONTA = :pRECEITA                                    '+
            '             THEN C.VALOR                                                  '+
            '           ELSE 0.0                                                        '+
            '           END) - SUM(CASE                                                 '+
            '           WHEN C.TIPOCONTA = :pDESPESA                                    '+
            '             THEN C.VALOR                                                  '+
            '           ELSE 0.0                                                        '+
            '           END)                                                            '+
            '       ), 0.0) * :pMULTIPLICAR AS VALOR                                    '+
            '   ,2 AS ORDEM                                                             '+
            ' FROM CONTA C                                                              '+
            ' WHERE NOT COALESCE(C.CONTAANUAL, FALSE)                                   '+
            '   AND NOT COALESCE(C.DEBITACARTAO, FALSE)                                 '+
            ...
            ' UNION                                                                     '+
            '                                                                           '+
            ' SELECT CAST(''Valor Reservado'' AS VARCHAR(40)) AS TITULO                 '+
            '   ,COALESCE(SUM(S.VALOR), 0.0) + (                                        '+
            '     SELECT COALESCE((                                                     '+
            '           SUM(CASE                                                        '+
            '               WHEN C.TIPOCONTA = :pRECEITA                                '+
            '                 THEN C.VALOR                                              '+
            ...
            ' UNION                                                                     '+
            '                                                                           '+
            ' SELECT CAST(''Projeção da Dívida'' AS VARCHAR(40)) AS TITULO              '+
            '   ,COALESCE((SUM(VALOR) / 12) * EXTRACT(MONTH FROM CURRENT_DATE), 0.0) + ('+
            '     (                                                                     '+
            '       SELECT COALESCE(SUM(VALOR) / 12, 0.0) AS VALOR                      '+
            '       FROM CONTA C                                                        '+
            '       WHERE C.TIPOCONTA = :pDESPESA                                       '+
            '         AND COALESCE(C.CONTAANUAL, FALSE)                                 '+
            '       ) * :pMULTIPLICAR                                                   '+
